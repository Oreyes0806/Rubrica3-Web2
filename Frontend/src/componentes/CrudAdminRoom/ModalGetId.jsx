import React, { useContext, useState } from 'react'
import { FormEditRoom } from './FormEditRoom'
import { HostalContext } from '../../context/Hostal/HostalContext'
import { useForm } from 'react-hook-form'

export const ModalGetId = ({setactualizarRoom}) => {
    const [Room, setRoom] = useState([])
    const {Rooms} = useContext(HostalContext)
    const {register, handleSubmit} = useForm()
  return (
    <div className='modal'>
        {
            !Room.length?(
                <form action='POST' onSubmit={handleSubmit(values=>{
                    if(Rooms){
                        setRoom(Rooms.filter(room=>room.idrooms == values.id))
                    }
              
            })} className='form'>
                <img className='close' onClick={()=>setactualizarRoom(false)} src="data:image/png;base64,"/>
                <h1 className='form_title'>Actualizar Room</h1>
                <div className='form_group'>                                   
                <label className='form_label' htmlFor="id">Id Room</label>
                <input className='form_input' type="number" {...register('id',{required:true})} />
                </div>
                <button className='btn'>Buscar y Editar</button>
            </form>
            ):(
              
                    <FormEditRoom setactualizarRoom={setactualizarRoom} room={Room}/>
              
            )
        }
        
    
       
    
    </div>
  )
}
