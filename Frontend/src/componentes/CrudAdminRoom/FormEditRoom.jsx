...
export const FormEditRoom = ({setactualizarRoom, room}) => {
    const {idrooms, numero, tipo, valor} = room[0]

    const {actualizarRoom} =useContext(HostalContext)
  ...
  return (
    <div>
        <form action="POST" onSubmit={handleSubmit(values=>{
            ...
            <button className='btn'>Actualizar Room</button>
        </form>
    </div>
  )
}
